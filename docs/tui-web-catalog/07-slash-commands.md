...
| /P-edit-prompt | `/edit-prompt` | | Minimal 外部编辑空草稿 | I-26；全屏可藏 | 否。**备注：外开编辑器 待讨论后再做。** |
| /P-expand | `/expand` | | Minimal 展开块 | 全屏用折叠手势 | 否（N/A）：全屏用折叠手势 |
...
| /P-privacy | `/privacy` | | coding data | A-12 | 是：打开设置（隐私开关） |
...
| /S-hooks-add | `/hooks-add` | | Hooks | 路径 | 同 | 否。**备注：见 15。** |
| /S-hooks-remove | `/hooks-remove` | | Hooks | 路径 | 同 | 否。**备注：见 15。** |
| /S-hooks-untrust | `/hooks-untrust` | | Hooks | | 同 | 否。**备注：见 15。** |
| /S-plugins | `/plugins` | `/plugin` | Plugins | list/reload/trust/add/remove/install/uninstall/update | 模态为主 | 否。**备注：见 15。** |
| /S-reload-plugins | `/reload-plugins` | | Plugins | = plugins reload | 同 | 否。**备注：见 15。** |
| /S-session-info | `/session-info` | `/status` `/info` | Always | | 同 | 是 |
| /S-feedback | `/feedback` | | Feedback | 关则藏 | 同 | 是 |
| /S-deep-research | `/deep-research` | | WorkflowLaunches | 立刻返回，进度 `/workflows` | 同 | 否。**备注：见 16。** |
| /S-workflow | `/workflow` | | WorkflowManagement | launch / pause/resume/stop/save | 同 | 否。**备注：见 16。** |
| /S-goal | `/goal` | | Goal | objective、`--budget`、status/pause/resume/clear | 同 | 否。**备注：见 16。** |
