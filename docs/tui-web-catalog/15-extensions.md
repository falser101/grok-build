...
Skills/MCP 不再是独立「扩展」层，并进设置后两栏。Skills 只读表：名字 / 来源 / 开|关（字不是开关）。MCP 只读：名字 / 状态（表头写「状态」+点）/ 工具数。空是「还没有」。`/skills` `/mcps` 和面板项已从斜杠拿掉；手打开设置对应栏。

| ID | 功能点 | TUI | Web 怎么做 | Web 已实现 |
|---|---|---|---|---|
| E-01 | MCP 列表 | `/mcps`；`x.ai/mcp/list` | 表：名、状态、工具数 | 是：设置 MCP 栏只读。名字 / 状态（点，表头写「状态」）/ 工具数。空则「还没有」。不进斜杠。 |
...
| E-09 | Skills 列表 | `/skills` tab | 列表 on/off、来源 local/user/plugin | 是：设置 Skills 栏只读。名字 / 来源 / 开\|关是字。空则「还没有」。不进斜杠。 |
