...
|---|---|---|---|---|---|
| Q-01 | `initialize` | C→A | W-11 A-04 | 第一条 | 是 |
| Q-02 | `authenticate` | C→A | A-05 | 登录 | 是 |
| Q-03 | `session/new` | C→A | S-01 | | 是 |
| Q-04 | `session/load` | C→A | S-03 重连 | `_meta.cursor` yolo/auto | 是：picker 不带 cursor；重连带 cursor |
...
| Q-06 | `session/cancel` | C→A | B-13 | Stop | 是 |
| Q-07 | `session/set_mode` | C→A | plan | P-06 | 是 |
| Q-08 | `session/setModel`? / 模型扩展 | C→A | `/model` | 以 initialize 与实测为准 | 是：`session/set_model` |
| Q-09 | `session/update` notif | A→C | 流 | 分发 C-/T- | 是 |
| Q-10 | `session/request_permission` | A→C 请求 | B-01 | **必须回 response** | 是 |
| Q-11 | `session/close` | C→A | 退出 | | 是：`x.ai/session/close` |
...
| Q-15 | AvailableCommandsUpdate | A→C | slash 刷新 | /K-03 | 是：刷新 slash |
...
| Q-22 | `x.ai/session/fork` | S-12 | | 是 |
| Q-23 | `x.ai/session/list` | picker/dashboard | | 是 |
| Q-24 | `x.ai/session/info` | /session-info | | 是 |
| Q-25 | `x.ai/session/close` | | | 是 |
| Q-26 | `x.ai/sessions/list` | roster | | 是 |
| Q-27 | `x.ai/session_search` | 搜内容 | | 是：`?` 前缀内容搜 |
| Q-28 | `x.ai/session/resolve_local_for_worktree_resume` | S-18 | | 是：`resume_session` |
| Q-29 | `x.ai/recap` | S-21 | | 是 |
| Q-30 | `x.ai/queue/remove` `reorder` `clear` `edit` `hold` `interject` | I-19 | | 部分：interject/clear；**备注：reorder/edit {NOTE}。** |
| Q-31 | `x.ai/interject` | 不取消的插入 | | 是 |
| Q-32 | `x.ai/rewind/*` | S-13 | 按扩展实际子方法 | 是：points + execute |
| Q-33 | `x.ai/prompt_history` | I-15 | | 是 |
| Q-34 | `x.ai/compact_conversation` | compact | | 是 |
| Q-35 | `x.ai/fs/list` `exists` `read_file` `write_file` | 文件 UI | @ 搜索后端之一 | 是：read_file 弹窗 |
| Q-36 | `x.ai/search/fuzzy/open` `change` | @ | | 是 |
| Q-37 | `x.ai/search/content` | | | 否。**备注：内容搜索扩展 待讨论后再做。** |
| Q-38 | `x.ai/git/status` `stage` `commit` `diffs` `discard` | | 可选 Git 面板 | 否。**备注：Git 面板 待讨论后再做。** |
| Q-39 | `x.ai/git/worktree/*` | S-17 | | 是：create/list/gc/resume |
| Q-40 | `x.ai/git/worktree/db/stats` `path` `rebuild` | CLI | 高级 | 否。**备注：worktree db 待讨论后再做。** |
| Q-41 | `x.ai/mcp/*` | E- | | 否。**备注：见 15。** |
| Q-42 | `x.ai/auth/get_url` `submit_code` `check_subscription` | A- | | 是 |
| Q-43 | `x.ai/feedback` | /feedback | | 是 |
| Q-44 | `x.ai/models/list` | 模型目录 | | 部分：session/info 模型列表 |
...
| Q-53 | `x.ai/terminal/create` `kill` `output` `wait_for_exit` | 后者拒绝 | 不宣称 terminal | 否（不做） |
| Q-54 | `x.ai/session/repair` | 历史修复 | 高级 | 否。**备注：session/repair 待讨论后再做。** |
| Q-55 | `x.ai/suggest/*` | 补全 | I-12 | 是 |

## x.ai 通知 A→C

| ID | 方法/update | TUI | Web | Web 已实现 |
|---|---|---|---|---|
| Q-60 | `x.ai/session_notification` | 大包 SessionUpdate 见下 | 按 tag 分发 | 是：按 sessionId 分流前后台 |
| Q-61 | `x.ai/session/update` | 与标准并行/replay | 同 C-34 | 是 |
| Q-62 | `x.ai/session/prompt_complete` | 回合结束 | 停 spinner、drain 队列 | 是：`prompt_complete` + 耐久 `turn_completed` 都清 `turnRunning` / 侧栏点 |
| Q-63 | `x.ai/queue/changed` | 共享队列真源 | I-19 | 是 |
| Q-64 | `x.ai/sessions/changed` | roster | D-27 | 是 |
| Q-65 | `x.ai/search/fuzzy/status` | @ 结果 | I-11 | 是 |
| Q-66 | `x.ai/search/content/status` | | | 否。**备注：content search status 待讨论后再做。** |
...
| Q-75 | `x.ai/models/update` | 目录变 | 刷新下拉 | 是：刷新模型下拉 |
| Q-76 | `x.ai/settings/update` | 远程设置 | 刷新设置页 | 部分：听设置/yolo 通知 |
| Q-77 | `x.ai/log` | 统一日志 | 不要在 UI 刷 | 是：不在 UI 刷 log |
| Q-78 | `x.ai/statusLine` 能力 | H- | 可选 | 是：initialize 宣称 statusLine |
| Q-79 | `x.ai/session/interjection` | 插话回声 | I-22 | 是：interject 芯片 |
