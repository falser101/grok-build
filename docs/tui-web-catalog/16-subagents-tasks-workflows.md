...
| X-11 | 后台 bash 任务 | `TaskBackgrounded` + tasks pane | 列表：命令、cwd、log | 否。**备注：子 agent/任务/工作流面板 待讨论后再做。** |
| X-12 | 任务完成 | `x.ai/task_completed` / `TaskCompleted` | 通知+行状态 | 否。**备注：子 agent/任务/工作流面板 待讨论后再做。** |
| X-13 | 杀任务 | `x.ai/task/kill` | 按钮 | 否。**备注：子 agent/任务/工作流面板 待讨论后再做。** |
| X-14 | 跟日志 | output_file tail；`task_output` 工具 | `<pre>` 轮询或通知流 | 否。**备注：子 agent/任务/工作流面板 待讨论后再做。** |
...
| X-17 | Monitor 使 Dashboard Working | D-04 | 同 | 否。**备注：子 agent/任务/工作流面板 待讨论后再做。** |
...
| X-32 | watching 状态 | 有后台时 turn 完仍 watching | 状态行 | 否。**备注：子 agent/任务/工作流面板 待讨论后再做。** |
